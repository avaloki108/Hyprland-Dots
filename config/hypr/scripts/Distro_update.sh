...
if command -v paru &> /dev/null || command -v yay &> /dev/null; then
  ...
  if command -v paru &> /dev/null; then
    alacritty --title update -e paru -Syu
    notify-send -i "$iDIR/ja.png" -u low 'Arch-based system' 'has been updated.'
  else
    alacritty --title update -e yay -Syu
    notify-send -i "$iDIR/ja.png" -u low 'Arch-based system' 'has been updated.'
  fi
elif command -v dnf &> /dev/null; then
  # Fedora-based
  alacritty --title update -e sudo dnf update --refresh -y
  notify-send -i "$iDIR/ja.png" -u low 'Fedora system' 'has been updated.'
elif command -v apt &> /dev/null; then
  # Debian-based (Debian, Ubuntu, etc.)
  alacritty --title update -e bash -c "sudo apt update && sudo apt upgrade -y"
  notify-send -i "$iDIR/ja.png" -u low 'Debian/Ubuntu system' 'has been updated.'
elif command -v zypper &> /dev/null; then
  # openSUSE-based
  alacritty --title update -e sudo zypper dup -y
